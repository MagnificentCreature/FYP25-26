        ...
        uint8_t wait, uint8_t send_stop)
{
    ...
    // PRINT("i2c_write_to: state = %d, in_repstart = %d\n", i2c_state, i2c_in_repstart);
    if (i2c_in_repstart == true) {
        ...
        // PRINT("i2c_write_to: START generated\n");
    }

    while(wait && (i2c_state == I2C_MTX)) {
        // PRINT("waiting...\n");
        // DelayMs(5000); // optional, to avoid flooding
        continue;
    }
    // PRINT("i2c_write_to: exit\n");
